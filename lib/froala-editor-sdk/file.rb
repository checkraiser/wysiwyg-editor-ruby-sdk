module FroalaEditorSDK
  ...
  class File
    ...
    @default_serve_path = "downloads/files"
    ...
    # Returns json object
    def self.upload(params, upload_path = @default_upload_path, serve_path = @default_serve_path, options = {})
      ...
      if file
        ...
        serve_url = save(file, path, serve_path)
    ...
    def self.save (file, path, serve_path)
      ...
      if ::File.open(path, "wb") {|f| f.write(file.read)}
        ...
        return "#{serve_path}#{Utils.get_file_name(path)}"
      else
        return "error"
      end
    end
